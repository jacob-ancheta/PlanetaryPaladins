...
public abstract class Interactable : MonoBehaviour
{
    ...
    public grabThrowObj attachedController { get; private set; }

    //Returns true if the controller was attached correctly.
    public bool TryAttach(grabThrowObj ctrl)
    {
        //Only attach if the controller is not holding something already.
        if (ctrl.interactibleInHand != null) return false;
    ...
    {
        OnEndInteraction();
        if (attachedController.interactibleInHand == this) attachedController.OnItemDetach(this);
        else Debug.LogError("Controller state (HeldItem) was incorrect. Tried to detach " + this + " while holding " + attachedController.interactibleInHand);
    ...
    public virtual void OnHoverEnter(grabThrowObj ctrl) { }

    //Called by WandController when an unattached controller overlaps this object's collider.
    public virtual void OnHoverExit(grabThrowObj ctrl) { }

    //Called by WandController on each frame an unattached controller overlaps this object's collider.
    public virtual void OnHoverStay(grabThrowObj ctrl) { }
